...
export class ChatGPT extends BaseScriptComponent {
  ...
  makeOCRRequest(imageTex: Texture, callback) {
    print("Making OCR request...");
    Base64.encodeTextureAsync(
      imageTex,
      (base64String) => {
        print("Image encode Success for OCR!");
        const ocrQuery =
          "Extract and return EXACTLY the visible text from the image. Reply only with the transcribed text and nothing else.";
        this.sendGPTChat(ocrQuery, base64String, callback);
      },
      () => {
        print("Image encoding failed for OCR!");
      },
      this.ImageQuality,
      this.ImageEncoding
    );
